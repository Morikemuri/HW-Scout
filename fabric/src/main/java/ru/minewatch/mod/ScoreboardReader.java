...
public class ScoreboardReader {
    ...
    public static final ScoreboardReader INSTANCE = new ScoreboardReader();
    ...
    private void tickInner(MinecraftClient mc) {
        ...
        if (sb != null) {
            for (Team team : sb.getTeams()) {
                ...
                String prefix = stripColors(team.getPrefix().getString());
                String suffix = stripColors(team.getSuffix().getString());
        ...
        // SCAN: armor stands in 64-block radius - group by X,Z position
        Box box = mc.player.getBoundingBox().expand(64);
        List<Entity> raw = mc.world.getOtherEntities(mc.player, box);
