...
public class MineHudRenderer extends DrawableHelper {
    ...
    public void onRenderHud(MatrixStack ms, float tickDelta) {
        ...
        // LEGEND ALERT: show popup only if filter allows legendary
        ...
        drawBtn(ms, font, btnResetX, btnResetY, BTN_W,  "x", C_BTN_RESET);
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            for (Map.Entry<String, MineData.ShaftEntry> se : e.getSortedShafts().entrySet()) {
                ...
                if (!shaft.hasTime() && shaft.currentType.isEmpty() && shaft.nextType.isEmpty()) continue;
                if (!shaftMatchesFilter(shaft, MineData.getFilterMode())) continue;
                cx += 4;
                drawCircle(ms, cx+1, ry+5, 2, worldColor(shaft.world));
                cx += 5;

                String curStr = shortType(shaft.currentType);
                if (!curStr.isEmpty()) {
                    font.draw(ms, curStr, cx, ry+3, mineColor(shaft.currentType, false));
                    cx += font.getWidth(curStr);
                }
                String nxtStr = shortType(shaft.nextType);
                if (!nxtStr.isEmpty()) {
                    font.draw(ms, ">", cx, ry+3, C_ARROW); cx += font.getWidth(">");
                    font.draw(ms, nxtStr, cx, ry+3, mineColor(shaft.nextType, true));
                    cx += font.getWidth(nxtStr);
                }
                int secs = shaft.hasTime() ? shaft.getRealSecs() : e.getMinSecs();
                if (secs >= 0) {
    ...
    private int computeWidth(TextRenderer font, List<MineData.MineEntry> entries) {
        ...
        for (MineData.MineEntry e : entries) {
            ...
            for (MineData.ShaftEntry shaft : e.getSortedShafts().values()) {
                if (!shaft.hasTime() && shaft.currentType.isEmpty() && shaft.nextType.isEmpty()) continue;
                if (!shaftMatchesFilter(shaft, MineData.getFilterMode())) continue;
                w += 4 + 5;
                w += font.getWidth(shortType(shaft.currentType));
                String nxt = shortType(shaft.nextType);
    ...
    // TICK: input poll, button dispatch, drag logic, alert fire
    public void onClientTick(MinecraftClient mc) {
        ...
        // CLICK: dispatch button actions on mouse release (no ralt)
        if (!mouseDown && mouseWasDown && !ralt) {
            if (inBtn(mx,my,btnPauseX,btnPauseY,BTN_W))  MineData.togglePause();
            if (inBtn(mx,my,btnResetX,btnResetY,BTN_W))  MineData.resetAll();
        ...
        // DRAG: ralt + LMB to reposition HUD panel
        List<MineData.MineEntry> entries = MineData.getSortedEntries();
        int totalH = HEADER_H + Math.max(1, entries.size()) * ROW_H + R;
        int x = MineData.getHudX(), y = MineData.getHudY();
        ...
        // ALERT: check threshold breach + dispatch SFX
        ...
        // LEGEND BELL: periodic audio if legend alert active + filter allows it
