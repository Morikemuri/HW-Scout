...
### v1.3.1
- Fix: snapshot parser no longer drops mines with valid timers due to CDN delay
- Fix: HUD entry order is now stable - entries with equal timers no longer swap positions each frame
- Fix: snapshot does not overwrite player-visited anarchy timers unless expired

### v1.3.0
- Mine timer HUD with multi-server support
- Telegram/Gist sync via Railway userbot
- Nether + End merged shaft display
- Legend alerts, filter modes, delete mode, draggable HUD

## Building from source

```bash
./gradlew build
# output: build/libs/minewatch-1.3.1-forge-1.16.5.jar
```
