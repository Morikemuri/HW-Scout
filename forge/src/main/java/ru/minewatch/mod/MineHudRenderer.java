...
// draws mine list, alert popup, handles btn clicks + drag
// ================================================================
public class MineHudRenderer {
    ...
    // RENDER: main HUD overlay - fires after all elements drawn
    @SubscribeEvent
    public void onRenderOverlay(RenderGameOverlayEvent.Post event) {
        ...
        if (MineData.hasLegendAlert() && MineData.getFilterMode() != 3)
            renderLegendAlert(ms, font, mc);
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            // SHAFTS: render each world shaft inline
    ...
    // WIDTH: compute dynamic HUD width from entry content
    private int computeWidth(FontRenderer font, List<MineData.MineEntry> entries) {
        ...
        for (MineData.MineEntry e : entries) {
            ...
            for (MineData.ShaftEntry shaft : e.getSortedShafts().values()) {
                ...
                if (!shaftMatchesFilter(shaft, MineData.getFilterMode())) continue;
                w += 4 + 5;
                w += font.width(shortType(shaft.currentType));
    ...
    // FILTER BTN: draw colored filter tab button
    ...
    // LEGEND ALERT: centered popup above hotbar+hearts area
    ...
    // TICK: input handler - mouse clicks + drag + sound alerts
    @SubscribeEvent
    public void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        // CLICK: button release handler (not in drag mode)
        ...
        // DRAG: RightAlt + left-click moves HUD
        List<MineData.MineEntry> entries = MineData.getSortedEntries();
        int totalH = HEADER_H + Math.max(1, entries.size()) * ROW_H + R;
        int x = MineData.getHudX(), y = MineData.getHudY();
        ...
        if (MineData.hasLegendAlert() && MineData.getFilterMode() != 3
                && mc.player != null && System.currentTimeMillis()%2000<60)
            mc.player.playSound(SoundEvents.NOTE_BLOCK_BELL, 1.0f, 1.8f);
    }

    // FILTER: check if shaft passes active rarity filter
    ...
    // COLOR: mine type color selector
    ...
    // COLOR: best rarity color for entry dot
    ...
    // DRAW: rounded rect (bottom corners only)
    ...
    // DRAW: filled rectangle via AbstractGui
    ...
    // SOUND: play pling alert scaled by urgency level
