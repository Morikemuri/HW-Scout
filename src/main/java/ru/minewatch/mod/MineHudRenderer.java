...
// ================================================================
// [MINEWATCH::HUD] :: overlay renderer + input handler
// renders mine status panel + dispatches audio alerts
// ================================================================
public class MineHudRenderer extends DrawableHelper {

    public static final MineHudRenderer INSTANCE = new MineHudRenderer();
    ...
    // -- EPIC ALERT COLORS --
    private static final int C_EPIC_BG   = 0xDD0A0014;
    private static final int C_EPIC_BORD = 0xFF9955EE;
    private static final int C_EPIC_TEXT = 0xFFDDBBFF;

    // -- INPUT STATE --
    private boolean mouseWasDown      = false;
    private boolean endWasDown        = false;
    private boolean cursorShownForHud = false; // true = we unlocked cursor via Right Alt
    ...
    // ROWS: {mode, rowX, rowY, rowW} for full-row delete-mode hit detection
    private final List<Object[]> rowXBtns = new ArrayList<>();

    // RENDER: full HUD overlay paint pass
    public void onRenderHud(MatrixStack ms, float tickDelta) {
        MinecraftClient mc   = MinecraftClient.getInstance();
        TextRenderer    font = mc.textRenderer;

        // HIDE: End key toggles full HUD visibility
        if (MineData.isHidden()) return;

        // ALERT: legend popup (suppressed on epic-only filter)
        if (MineData.hasLegendAlert() && MineData.getFilterMode() != 3) renderLegendAlert(ms, font, mc);
        // ALERT: epic popup (suppressed on legend-only filter, or when legend already shown)
        else if (MineData.hasEpicAlert() && MineData.getFilterMode() != 2) renderEpicAlert(ms, font, mc);
        ...
        // X btn: red when delete mode active, normal otherwise
        drawBtn(ms, font, btnResetX, btnResetY, BTN_W,  "x", MineData.isDeleteMode() ? 0xFFFF2200 : C_BTN_RESET);
        ...
        // SERVER COUNT: displayed left of filter btn
        String cnt = entries.size() + " серв.";
        font.draw(ms, cnt, btnFiltX-font.getWidth(cnt)-3, y+3, C_SUB);

        // BODY: divider + rounded-bottom background
        ...
        if (MineData.isPaused()) {
            String s = "[ пауза ]";
            font.draw(ms, s, x+(W-font.getWidth(s))/2, y+HEADER_H+3, 0x77AAAAAA); return;
        }

        // ROWS: per-server shaft data
        rowXBtns.clear();
        int ry = y + HEADER_H;
        for (int i = 0; i < entries.size(); i++) {
            ...
            if (isActive) {
                ...
                drawRect(ms, x, ry, 2, ROW_H, C_ACT_BAR);  // active server bar
            } else if (e.remoteOnly) {
                drawRect(ms, x, ry, 2, ROW_H, C_REMOTE_BAR); // remote node bar
            }

            drawCircle(ms, x+PAD+2, ry+5, 3, bestMineColor(e)); // rarity indicator dot

            String label = e.mode.replace("Лайт","").replace("лайт","").trim();
            font.draw(ms, label, x+PAD+9, ry+3, isActive ? C_MODE_A : C_MODE);
            int cx = x + PAD + 9 + font.getWidth(label);

            // SHAFTS: inline world dot + type labels + timer
            // Ад+Энд are displayed as one merged shaft
            for (Map.Entry<String, MineData.ShaftEntry> se : e.getSortedShafts().entrySet()) {
                ...
                // Ад: absorb Энд data (take non-empty type, min timer)
                ...
                // filter by merged types (active server always shown)
                if (!isActive) {
                ...
                if (!curStr.isEmpty()) {
                    ...
                    cx += font.getWidth(curStr);
                }
                String nxtStr = shortType(nxtType);
                if (!nxtStr.isEmpty()) {
                    font.draw(ms, ">", cx, ry+3, C_ARROW); cx += font.getWidth(">");
                    font.draw(ms, nxtStr, cx, ry+3, mineColor(nxtType, true));
                    cx += font.getWidth(nxtStr);
                }
                if (secs < 0) secs = e.getMinSecs();
                if (secs >= 0) {
                    String t = " " + MineData.fmt(secs);
                    int tc = secs > 60 ? C_TIME_OK : secs > 30 ? C_TIME_WRN : C_TIME_CRT;
                    font.draw(ms, t, cx, ry+3, tc); cx += font.getWidth(t);
                }
            }

            // DELETE MODE: highlight row + show X marker; store row bounds for click detection
            if (!isActive) {
                rowXBtns.add(new Object[]{e.mode, x, ry, W});
                if (MineData.isDeleteMode()) {
                    drawRect(ms, x, ry, W, ROW_H, 0x22FF2200);
                    font.draw(ms, "x", x+W-BTN_W, ry+3, 0xCCFF3322);
                }
            }
        ...
        if (MineData.isDragging())
            font.draw(ms, "[ двигай ]", x+(W-font.getWidth("[ двигай ]"))/2, y+3, 0x88FFFFFF);
    }

    // CALC: minimum panel width to fit all content
    private int computeWidth(TextRenderer font, List<MineData.MineEntry> entries) {
        int min = PAD + font.getWidth("MineWatch") + 4
                + font.getWidth("15 серв.") + BTN4_W + BTN3_W + BTN_W*3 + PAD + 30;
        int max = min;
        String activeMode = MineData.getAnarchyMode();
        for (MineData.MineEntry e : entries) {
            boolean isActiveEntry = e.mode.equals(activeMode);
            int w = PAD + 9 + font.getWidth(e.mode.replace("Лайт","").replace("лайт","").trim());
            for (Map.Entry<String, MineData.ShaftEntry> se : e.getSortedShafts().entrySet()) {
                String shWorld = se.getKey();
                if ("Энд".equals(shWorld) && e.shafts.containsKey("Ад")) continue;
                MineData.ShaftEntry shaft = se.getValue();
                ...
                if (!shaft.hasTime() && curType.isEmpty() && nxtType.isEmpty()) continue;
                if (!isActiveEntry) {
                    int rank = Math.max(typeRank(curType), typeRank(nxtType));
                    int fm   = MineData.getFilterMode();
                    if (fm == 1 && rank < 1) continue;
                ...
                w += 4 + 5;
                w += font.getWidth(shortType(curType));
                String nxt = shortType(nxtType);
                if (!nxt.isEmpty()) w += font.getWidth(">") + font.getWidth(nxt);
                w += font.getWidth(" 15:00");
            }
            w += PAD;
            max = Math.max(max, w);
        }
    ...
    // DRAW: filter button with mode-specific coloring
    private void drawBtnFilter(MatrixStack ms, TextRenderer font, int bx, int by, int bw, int mode) {
        drawRR(ms, bx-1, by-1, bw+2, BTN_H+2, 2, 0x22FFFFFF);
        if (mode == 1) {
            // EL mode: split purple/gold fill
        ...
        font.draw(ms, label, bx+(bw-font.getWidth(label))/2, by+1, lc);
    }

    // RENDER: center-screen legend alert popup
    private void renderLegendAlert(MatrixStack ms, TextRenderer font, MinecraftClient mc) {
        String info = MineData.getLegendAlertText();
        if (info == null) return;
        String line1 = "Легендарная шахта";
        int sw = mc.getWindow().getScaledWidth();
        int sh = mc.getWindow().getScaledHeight();
        int bw = Math.max(font.getWidth(line1), font.getWidth(info)) + 24;
        int bh = 28;
        int bx = (sw - bw) / 2;
        int by = sh - bh - 55; // above hotbar + hearts/food area
        drawRR(ms, bx-1, by-1, bw+2, bh+2, 4, C_LEG_BORD);
        drawRR(ms, bx,   by,   bw,   bh,   4, C_LEG_BG);
        drawRect(ms, bx+3, by+1, bw-6, 1, C_LEG_BORD);
        font.draw(ms, line1, bx+(bw-font.getWidth(line1))/2, by+4,  C_LEG_BORD);
        font.draw(ms, info,  bx+(bw-font.getWidth(info))/2,  by+15, C_LEG_TEXT);
    }

    // EPIC ALERT: same position as legend alert, purple color scheme
    private void renderEpicAlert(MatrixStack ms, TextRenderer font, MinecraftClient mc) {
        String info = MineData.getEpicAlertText();
        if (info == null) return;
        String line1 = "Эпическая шахта";
        int sw = mc.getWindow().getScaledWidth();
        int sh = mc.getWindow().getScaledHeight();
        int bw = Math.max(font.getWidth(line1), font.getWidth(info)) + 24;
        int bh = 28;
        int bx = (sw - bw) / 2;
        int by = sh - bh - 55;
        drawRR(ms, bx-1, by-1, bw+2, bh+2, 4, C_EPIC_BORD);
        drawRR(ms, bx,   by,   bw,   bh,   4, C_EPIC_BG);
        drawRect(ms, bx+3, by+1, bw-6, 1, C_EPIC_BORD);
        font.draw(ms, line1, bx+(bw-font.getWidth(line1))/2, by+4,  C_EPIC_BORD);
        font.draw(ms, info,  bx+(bw-font.getWidth(info))/2,  by+15, C_EPIC_TEXT);
    }

    // DRAW: standard button
    private void drawBtn(MatrixStack ms, TextRenderer font, int bx, int by, int bw, String label, int lc) {
        drawRR(ms, bx-1, by-1, bw+2, BTN_H+2, 2, 0x22FFFFFF);
        drawRR(ms, bx,   by,   bw,   BTN_H,   2, C_BTN);
        font.draw(ms, label, bx+(bw-font.getWidth(label))/2, by+1, lc);
    }

    // TICK: input handler - cursor mgmt, clicks, drag, sound alerts
    public void onClientTick(MinecraftClient mc) {
        ...
        // KEY: End -> toggle HUD visibility
        if (endDown && !endWasDown) MineData.toggleHidden();
        endWasDown = endDown;

        // CURSOR: Right Alt unlocks cursor so HUD can be interacted with / dragged
        // When Right Alt released while in-game, re-grab cursor
        if (ralt && mc.currentScreen == null) {
            if (!cursorShownForHud) {
                GLFW.glfwSetInputMode(win, GLFW.GLFW_CURSOR, GLFW.GLFW_CURSOR_NORMAL);
                cursorShownForHud = true;
            }
        } else if (cursorShownForHud && mc.currentScreen == null) {
            GLFW.glfwSetInputMode(win, GLFW.GLFW_CURSOR, GLFW.GLFW_CURSOR_DISABLED);
            cursorShownForHud = false;
        }
        // Cursor is usable for hit-testing when: we unlocked it OR a screen is already open
        boolean cursorVisible = cursorShownForHud || mc.currentScreen != null;
        ...
        // CLICK: cursor visible (screen open OR Right Alt) → button clicks work
        // Drag is NOT active when only screen open (no Right Alt) so isDragging is always false here
        if (!mouseDown && mouseWasDown && !MineData.isDragging() && !MineData.isHidden() && cursorVisible) {
            ...
            if (MineData.isDeleteMode()) {
                for (Object[] row : rowXBtns) {
                    int rx=(int)row[1], ry=(int)row[2], rw=(int)row[3];
                    if (mx>=rx && mx<=rx+rw && my>=ry && my<ry+ROW_H) {
                        MineData.removeEntry((String)row[0]);
        ...
        // DRAG: ONLY with Right Alt (explicit drag intent, no conflict with button clicks)
        List<MineData.MineEntry> entries = MineData.getSortedEntries();
        int totalH = HEADER_H + Math.max(1, entries.size()) * ROW_H + R;
        int x = MineData.getHudX(), y = MineData.getHudY();
        if (ralt && mouseDown && cursorVisible) {
            if (!mouseWasDown) {
                if (mx>=x && mx<=x+lastW && my>=y && my<=y+totalH) MineData.startDrag(mx, my);
            } else if (MineData.isDragging()) {
                MineData.updateDrag(mx, my);
            }
        } else if (!mouseDown || !ralt) {
            MineData.stopDrag();
        }
        mouseWasDown = mouseDown;

        // SOUND: proximity alerts at 60/30/10s thresholds
        ...
        // BELL: legend alert pulse (suppressed on epic-only filter)
        if (MineData.hasLegendAlert() && MineData.getFilterMode() != 3 && mc.player != null && System.currentTimeMillis()%2000<60)
            mc.player.playSound(SoundEvents.BLOCK_NOTE_BLOCK_BELL, 1.0f, 1.8f);
        // BELL: epic alert pulse (suppressed on legend-only filter)
        if (MineData.hasEpicAlert() && MineData.getFilterMode() != 2 && mc.player != null && System.currentTimeMillis()%2000<60)
            mc.player.playSound(SoundEvents.BLOCK_NOTE_BLOCK_PLING, 1.0f, 1.5f);
    }

    // FILTER: shaft rarity vs active filter mode
    // 0=all  1=epic+  2=leg only  3=epic only
    ...
    // ABBREV: short display label for mine type
    ...
    // COLOR: best rarity color for server dot
    ...
    // RANK: 0=ord 1=epic 2=myth 3=leg
    ...
    // SFX: threshold breach audio signal - escalating intensity
    private void playAlert(MinecraftClient mc,int i) {
        if(mc.player==null) return;
        switch(i) {
            case 0: mc.player.playSound(SoundEvents.BLOCK_NOTE_BLOCK_PLING,0.5f,1.0f); break; // 60s warn
            case 1: mc.player.playSound(SoundEvents.BLOCK_NOTE_BLOCK_PLING,0.8f,1.5f); break; // 30s warn
            case 2: mc.player.playSound(SoundEvents.BLOCK_NOTE_BLOCK_PLING,1.0f,2.0f);        // 10s crit x2
                    mc.player.playSound(SoundEvents.BLOCK_NOTE_BLOCK_PLING,1.0f,2.0f); break;
        }
    }
}
